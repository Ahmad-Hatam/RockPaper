...
class MipsTest:
    ...
    def execute(self, marsPath=MARS_PATH, mars_flags=mars_flags):
        ...
        arg = shlex.split(f"{marsPath} {flags_str} {sources_str}")
        return subprocess.run(arg, check=False, capture_output=True)
    ...
    def run_test(self, marsPath=MARS_PATH, mars_flags=mars_flags, get_runtime=False):
        t = self.execute(marsPath=marsPath, mars_flags=mars_flags)
        ret = t.returncode
        output = t.stdout
        if ret != 0:
        ...
        if diffResult.returncode == 0:
            time_measure = self.execute_for_runtime(marsPath=marsPath, mars_flags=mars_flags) if get_runtime else None
